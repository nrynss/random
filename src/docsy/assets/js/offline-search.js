...
/* Added custom behaviour for search result page
 *  1. Open search results in new page.
 *  2. Highlight search words
 */
  ...
  $(document).ready(function () {
    ...
    const render = ($targetSearchInput) => {
      ...
      const $searchResultBody = $("<div>").css({
        maxHeight: `calc(100vh - ${$targetSearchInput.offset().top - $(window).scrollTop() + 180
          }px)`,
        overflowY: "auto",
      });
      ...
      if (results.length === 0) {
        ...
        results.forEach((r) => {
          ...
          //Custom code to highlight search results
          const sentence = doc.excerpt.replaceAll(
            searchQuery,
            `<label style="background-color: #FFFF27;">${searchQuery}</label>`
          );
          $entry.append($("<p>").html(sentence));
